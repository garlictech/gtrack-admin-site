...
import { AUTH_CONFIG_TOKEN, AuthenticationApiConfig, MagiclinkConfig } from '../lib/config';
...
export class PasswordlessService {
  ...
  @DebugLog callback(passwordlessToken: string, uid: string, roles?: Array<string>): Observable<Auth> {
    return this.http
      .post<{
        token: string;
        refreshToken: string;
