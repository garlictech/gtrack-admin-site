...
export class ObjectMarkEffects {
  ...
  public loadContext$: Observable<Action> = this._actions$
    ...
    .pipe(
      mergeMap(action => {
        return this._objectMark
          .loadContext(action.context)
          .pipe(
            take(1),
            map(objects =>
              new LocalActions.ContextLoaded(action.context, objects)
            )
          )
      })
    );
    ...
    public markObject$: Observable<Action> = this._actions$
      ...
      .pipe(
        mergeMap(action => {
          return this._objectMark
            .mark(action.context, action.object, action.mark)
            .pipe(
              take(1),
              map(() =>
                new LocalActions.ObjectMarked(action.context, action.object, action.mark)
              )
            )
        })
      );
  ...
  constructor(
    ...
    private _objectMark: ObjectMarkService
  ) {}
}
