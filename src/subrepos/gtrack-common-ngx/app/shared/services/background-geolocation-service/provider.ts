...
export class BackgroundGeolocationService {
  ...
  public start() {
    ...
    const geolocation$ = Observable.timer(0, 60000)
      .pipe(
        mergeMap(() => {
        ...
        catchError(err => {
          log.er('Current location error: ', err);
          return throwError(err);
        }),
        retry()
      );

    const geoip$ = this._geoip
      .getLocation()
      .pipe(
        take(1),
        ...
        tap(body => log.d('GeoIP result: ', body)),
    ...
    .pipe(
      map(result =>
        this._store.dispatch(new Actions.CurrentLocationObtained(result))
      )
    )
    .subscribe();
