...
export class Effects {
  // Twitter login
  @Effect()
  twitterLogin$: Observable<Action> = this._actions$.pipe(
  ...
  @Effect()
  requestVerifyToken$: Observable<Action> = this._actions$.pipe(
  ...
  // Verify
  @Effect()
  verify$: Observable<Action> = this._actions$.pipe(
  ...
  // Google login
  @Effect()
  googleLogin$: Observable<Action> = this._actions$.pipe(
    ofType<LocalActions.GoogleLogin>(LocalActions.GOOGLE_LOGIN),
    switchMap(action => {
      log.data('Effect: Google login initiated');
      return observableFrom(this._google.connect(action.roles)).pipe(
        map(auth => {
  ...
  // Facebook login
  @Effect()
  facebookLogin$: Observable<Action> = this._actions$.pipe(
  ...
  // Magic Link login
  @Effect()
  magiclinkRequestToken$: Observable<Action> = this._actions$.pipe(
  ...
  // Magic Link login
  @Effect()
  magiclinkLogin$: Observable<Action> = this._actions$.pipe(
  ...
  @Effect()
  unauthorized$: Observable<Action> = this._actions$.pipe(
    ofType<LocalActions.Unauthorized>(LocalActions.UNAUTHORIZED),
    switchMap(() => {
      return observableFrom(this._auth.refreshToken()).pipe(
        map(auth => {
          return new LocalActions.LoginSuccess(auth);
        }),
        catchError(err => {
