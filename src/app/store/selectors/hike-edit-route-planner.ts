...
export class HikeEditRoutePlannerSelectors {
  ...
  constructor() {
    ...
    this.getRoutePlanner = createSelector(this._featureSelector, (state: IHikeEditRoutePlannerState) => state);

    this.getRoute = createSelector(this._featureSelector, (state: IHikeEditRoutePlannerState) => state.route);

    this.getPath = createSelector(
      this._featureSelector,
      (state: IHikeEditRoutePlannerState) => state.route.features[0]
    );

    this.getPathLength = createSelector(
      this._featureSelector,
      (state: IHikeEditRoutePlannerState) => _get(state.route.features[0], 'geometry.coordinates', []).length
    );

    this.getSegments = createSelector(this._featureSelector, (state: IHikeEditRoutePlannerState) => state.segments);

    this.getTotal = createSelector(this._featureSelector, (state: IHikeEditRoutePlannerState) => state.total);

    this.getIsRoundTrip = createSelector(this._featureSelector, (state: IHikeEditRoutePlannerState) => {
    ...
    this.getIsPlanning = createSelector(this._featureSelector, (state: IHikeEditRoutePlannerState) => state.planning);

    this.getIsRouting = createSelector(this._featureSelector, (state: IHikeEditRoutePlannerState) => state.routing);
  }
}
