...
import { EAuthRoles } from 'subrepos/provider-client/interfaces';
...
import { publicProfileStateAdapter, IPrivateProfileState, ISettingsState, IHikeProgramSettingsState } from './state';
...
const selectFeature = createFeatureSelector<ISettingsState>('settings');

const selectPrivateProfileFeature = createSelector(selectFeature, (state: ISettingsState) => state.privateProfile);
...
export const profileFormDataPath = (group: EProfileGroup) =>
  createSelector(AuthenticationSelectors.selectRole, role => `settings.privateProfile.data.profile.${group}`);

export const profileGroupSelector = (group: EProfileGroup) =>
  createSelector(selectPrivateProfileFeature, (state) =>
    _get(state, `data.profile.${group}`)
  );

export const profileGroupSaveFailure = (group: EProfileGroup) =>
  createSelector(selectPrivateProfileFeature, (state: IPrivateProfileState) => state.profileGroups[group].failed);

export const profileGroupSaving = (group: EProfileGroup) =>
  createSelector(selectPrivateProfileFeature, (state: IPrivateProfileState) => state.profileGroups[group].working);

export const selectPrivateProfileInCurrentRole = createSelector(
  AuthenticationSelectors.selectRole,
  selectPrivateProfileFeature,
  (role, state) => _get(state, `data.profile`)
);

const publicProfilesSelector = createSelector(selectFeature, (state: ISettingsState) => state.publicProfiles);

const { selectEntities: selectPublicUserEntities } = publicProfileStateAdapter.getSelectors(publicProfilesSelector);

const selectPublicDataOf = userId => createSelector(selectPublicUserEntities, entities => entities[userId]);
...
export class Selectors {
  ...
  getPublicProfileOf(userId: string, role: EAuthRoles) {
    return this.getPublicDataOf(userId, role).pipe(
      switchMap(data => {
        let selectedProfile: IDecoratedPublicCommonProfile = _cloneDeep(_get(data, 'profile'));
